...
filtered_p <- dplyr::filter(penguins, sex == "female", culmen_length_mm >= 39, island == "Dream" | island == "Torgerson")
...
no_com <- penguins %>% dplyr::select(-comments)
...
```{r, message=FALSE, warning=FALSE}
id_push <- penguins %>% dplyr::relocate(individual_id, .after = sample_number)
...
isl_nm <- penguins %>% dplyr::rename(island_name = island)
...
```{r, message=FALSE, warning=FALSE}
mutate <- penguins %>% dplyr::mutate(flippometers = flipper_length_mm/1000000)
...
number8 <- penguins %>% 
  ...
  summarise(culmen_depth_mean = mean(culmen_depth_mm, na.rm = TRUE),
...
fnl_boss <- penguins %>%
  ...
  filter(sex == "FEMALE",!is.na(flipper_length_mm), island == "Dream" |
           island == "Biscoe") %>% 
