...
mtcars_group$mpg <- as.factor(mtcars_group$mpg)

ggplot(mtcars_group, aes(x = "mpg", y = n, fill = "mpg")) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar(theta = "y", start = pi/3) +
  scale_fill_manual(values = rainbow(nrow(mtcars_group))) +
  geom_text(aes(label = "cyl"), position = position_stack(vjust = 0.5), size = 2, angle = 45) + 
