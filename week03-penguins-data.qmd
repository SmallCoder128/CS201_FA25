...
gentoo_data <- read.csv(url_gentoo)
...
```{r, message=FALSE}
ggplot(data = penguins_data, aes(x = culmen_depth_mm, color = species)) +
  geom_histogram() +
  theme_bw() +
...
plot1 <- ggplot(data = penguins_data, aes(x = culmen_depth_mm, y = body_mass_g, color = species)) +
  geom_point() +
  theme_bw() +
  scale_fill_viridis_d()+
  labs(title = "Palmer Penguins, Distribution of Culmen Depth",
