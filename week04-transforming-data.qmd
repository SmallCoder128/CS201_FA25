...
```{r, message=FALSE}
plot2 <-ggplot(data = penguins_male_female, aes(x = flipper_length_mm, y = body_mass_g, color = species, shape = island)) +
  geom_point() +
  theme_bw() +
  scale_color_viridis_d() +
  facet_grid(island ~ sex) + # here is where we designate faceting!
   labs(title = "Palmer Penguins, Flipper Length & Body Mass",
       x = "flipper length (mm)",
       y = "body mass (g)")
...
2.  Now, create a bar graph using the *new* dataframe you made
    ...
    ```{r}
    ggplot(data = average_mass , aes(x = average_body_mass, y = species)) + # fill in the blanks
...
```{r, message=FALSE}
plot3 <- ggplot(data = average_flipper_length, aes(x = average_flipper_length, y = species, color = species))+
  geom_bar(stat = "identity") +
  labs(title = "Average Flipper Length")
...
```{r, message=FALSE}
plot4 <- ggplot(data = penguins_data, aes(x = species, y = body_mass_g, color = species))+
  geom_boxplot()
...
culmen_plot <- ggplot(data = average_culmen_depth, aes(x= species, y = average_culmen_depth, fill = species)) +
  geom_bar(stat = "identity")+
  coord_flip()
